...
"""
set count_greater_than_two = 0

for each line 
    
    if x1=x2 then:
        increasing y from y1 to y2
        for each point it passes through add one
        if point = 2 increase count_greater_than_two by 1
        
        
    if y1=y2 then:
        increasing x from x1 to x2
        for each point it passes through add one
        if point = 2 increase count_greater_than_two by 1
"""
...
def get_vents():
    ...
    vents = [[tuple([int(i) for i in vent_end.split(',')]) for vent_end in vent_ends] for vent_ends in data_strings[0:-1]]
    return vents
