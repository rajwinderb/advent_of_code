...
"""
read each line and save the number for each position in list of lists
list of 5 lists [<number of 0's>, <number of 1's>]

then make gamma from the most common of each
then make the epsilon from the least common of each 
...
def main():
    ...
    with open('../inputs/three_puzzle_one_input.txt') as file:
        while line := file.readline().rstrip():
            counts = count_vals(counts, list(line))

    gamma = ""
    epsilon = ""
    for count in counts:
        gamma += str(count.index(max(count)))
        epsilon += str(count.index(min(count)))
