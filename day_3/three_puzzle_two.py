...
def find(most):
    with open('../inputs/three_puzzle_one_input.txt') as f:
        lines = f.read().splitlines()

    # range(5) for test, range(12) for input
    for i in range(12):
        count_values = Counter(line[i] for line in lines)
        if most:
            wanted_val = 1 if count_values['1'] >= count_values['0'] else 0
        else:
            wanted_val = 0 if count_values['1'] >= count_values['0'] else 1

        lines = [line for line in lines if line[i] == str(wanted_val)]
...
def main():
    oxygen_generator_rating = find(True)
    co2_scrubber_rating = find(False)
