...
def change_position(move, horizontal, depth, aim):
    if move[0] == "forward":
...
def main():
    ...
    depth = 0
    aim = 0
    with open('../inputs/two_puzzle_one_input.txt') as file:
        while line := file.readline().rstrip().split():
            horizontal, depth, aim = change_position(line, horizontal, depth, aim)
